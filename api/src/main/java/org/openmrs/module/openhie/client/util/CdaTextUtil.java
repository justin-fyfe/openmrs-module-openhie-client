...
public final class CdaTextUtil {
	...
	public StructDocElementNode generateText(ClinicalStatement statement, StructDocElementNode context)
	{
		if(statement.isPOCD_MT000040UVOrganizer())
			...
			return this.generateText((Act)statement, context);
		else if(statement.isPOCD_MT000040UVSubstanceAdministration())
			return this.generateText((SubstanceAdministration)statement, context);
		else
	 ...
	 * </tr>
	 */
	public StructDocElementNode generateText(SubstanceAdministration administration, StructDocElementNode context)
	{
	 ...
	 * Text for Acts
	 */
	public StructDocElementNode generateText(Act act, StructDocElementNode context)
	{
		for(EntryRelationship er : act.getEntryRelationship())
		{
			...
			StructDocElementNode node = this.generateText(er.getClinicalStatement(), context);
	...
	public StructDocElementNode generateText(Observation observation, StructDocElementNode context)
	{
		...
		else if(observation.getValue() instanceof TS)
		{
			TS data = (TS)observation.getValue();
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MMM-dd");
			valueTd = retVal.addElement("td", sdf.format(data.getDateValue().getTime()));
		}
		...
		{
			authorCol.getChildren().add(this.generateAuthorDisplay(aut));
		...
		if(observation.getEntryRelationship().size() > 0)
		{
			...
			for(EntryRelationship er : observation.getEntryRelationship())
			{
				...
				StructDocElementNode row = this.generateText(er.getClinicalStatement(), subContext);
	 ...
	 */
	private StructDocTextNode generateAuthorDisplay(Author authorInfo)
	{
		// Find the author
		if(authorInfo != null)
		{
