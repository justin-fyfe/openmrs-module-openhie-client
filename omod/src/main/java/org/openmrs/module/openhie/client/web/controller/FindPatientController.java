 ...
 */
@Controller
@RequestMapping("/module/openhie-client/findPatient")
...
public class FindPatientController {
	...
	public ModelAndView doSearch(Map<String, Object> model, @ModelAttribute("patientSearch") PatientSearchModel search) throws ParseException
	{
		...
		try {
			model.put("results", service.searchPatient(search.getFamilyName(), search.getGivenName(), dobDate, isFuzzy, search.getGender(), identifier, momsIdentifier));
			model.put("patientSearch", search);
			return new ModelAndView("findPatient", model);
